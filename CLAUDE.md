...
산출물은 LoRA 어댑터 하나(125MB)다. 추론은 이 저장소가 하지 않는다 — `../Detection-Server`가
...
- **모델에게 숫자를 묻지 말 것.** 확률이든 점수든 단계든, 생성으로 뽑은 숫자는 학습으로
  점검된 적이 없다. 진행 단계를 1~4로 물어봤다가 거의 전부 3으로 쏠리는 것을 확인했다
  (`docs/RESULTS.md`). 숫자는 **로짓에서 읽거나 규칙으로 계산**한다.
...
│
├── adapter/                    # 학습 산출물 (gitignore, prompt/calibration만 예외)
│   ├── adapter_model.safetensors   125MB
...
- 베이스 모델 `google/gemma-3-4b-it` (게이트됨 — 승인 필요)
...
python3 common.py --write-contract adapter/   # 기존 어댑터에 계약 붙이기
```

**Colab에서** — GPU가 필요한 학습·평가. 처음이면 `docs/COLAB.md`를 그대로 따라간다.

```bash
python3 train_lora.py                        # 1 epoch, T4에서 약 2시간 40분
...
- **어댑터(adapter)** — LoRA 가중치. 원본 Gemma는 얼려두고 이것만 학습한다. 별개 파일이라
  껐다 켤 수 있고, 끄면 원본 Gemma로 돌아간다(근거 생성에 이용).
